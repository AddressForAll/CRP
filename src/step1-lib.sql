...
CREATE or replace FUNCTION crp.from_cep(cep text) RETURNS text AS $f$
  ...
  --prefExtra2UF_dg int[]  = array[6,   7,   7,   6,   8,   8];
  ...
  IF cep is null OR length(cep)!=8  THEN
     RETURN '(ERRO 10)'; -- CEP vazio ou com tamanho inválido
  END IF;
  m := regexp_match(cep,prefExtra_rgx);
  aux := array_count_first_notnull(m);
  IF aux>0 THEN
    uf := prefExtra2UF[aux];
    pref = array_first_notnull(m);
  ELSE
    BEGIN
    m := regexp_match(cep,prefMain_rgx);
	IF m is not NULL THEN
      pref := m[1];
	  --RAISE NOTICE 'pref (%)', pref;
      uf := prefMain2uf->>pref;
	ELSE
	  RETURN '(ERRO 20)'; -- CEP vazio ou com
	END IF;
    END;
  END IF;
  ...
  END
$f$ LANGUAGE PLpgSQL IMMUTABLE;
-- COMMENT FUNCTION IS 'Encodes CEP as CRP';

CREATE or replace FUNCTION crp.from_cep(cep int) RETURNS text AS $f$
